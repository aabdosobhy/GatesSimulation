...
{
	int Cx = GInfo.x1 + UI.XOR2_Width / 2;
	int Cy = GInfo.y1 + UI.XOR2_Height / 2;
...
void XOR2::move(Output * pOut, GraphicsInfo kkk)
{
	pOut->DrawXOR2(kkk, true);

}
void XOR2::copy(ApplicationManager *pManager, int x, int y, vector<int>& iddones2)
{
	GraphicsInfo GInfo;
	GInfo.x1 = x;
	GInfo.y1 = y;
...
void XOR2::cut(ApplicationManager *pManager, int x, int y, vector<int>& iddones2)
{
