#include "AND3.h"
//#include"..\ApplicationManager.h" // test1// include malosh lazma
...
{
	int Cx = GInfo.x1 + UI.AND3_Width / 2;
	int Cy = GInfo.y1 + UI.AND3_Height / 2;
...
void AND3::move(Output * pOut, GraphicsInfo kkk)
{
	pOut->DrawAND3(kkk, true);
...
void AND3::copy(ApplicationManager *pManager, int x, int y, vector<int>& iddones2)
{
	GraphicsInfo GInfo;
	GInfo.x1 = x;
	GInfo.y1 = y;
...
void AND3::cut(ApplicationManager *pManager, int x, int y, vector<int>& iddones2)
{
